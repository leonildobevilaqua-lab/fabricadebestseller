...
const LandingPage: React.FC<LandingProps> = ({ onStart, onAdmin, lang, setLang, initialState, onLoginClick }) => {
    ...
    const processingMessages = [
        "Enviando dados para a equipe criativa...",
        "Conectando com a Inteligência Artificial...",
        "Estruturando os capítulos do seu livro...",
        "Diagramando o conteúdo profissionalmente...",
    ...
    const handleSaveLead = async () => {
        try {
            await fetch('/api/payment/leads', {
                ...
                body: JSON.stringify({ ...formData, ...bookData, tag: 'Id_avulso_pt' })
            });
        } catch (e) {
        ...
        <div className="min-h-screen font-sans bg-slate-900 text-slate-100 selection:bg-yellow-500 selection:text-slate-900 overflow-x-hidden">
                    ...
                    <div className="min-h-screen flex flex-col">
                        <div className="p-6 flex justify-between items-center border-b border-slate-800">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-bold">Criar Meu Best Seller</span>
                            </div>
                            ...
                            <div className="w-full max-w-xl">
                                {step === 0 && (
                                    <div className="space-y-6">
                                        <h2 className="text-2xl font-bold text-center">Dados Pessoais</h2>
                                        <div className="space-y-4 bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
                                            <input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-slate-900 border-slate-700 rounded-xl p-3" placeholder="Nome Completo" />
                                            <input value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-slate-900 border-slate-700 rounded-xl p-3" placeholder="E-mail" />
                                            <input value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full bg-slate-900 border-slate-700 rounded-xl p-3" placeholder="WhatsApp" />
                                            <label className="flex items-start gap-3 cursor-pointer">
                                                ...
                                                <span className="text-xs text-slate-400">Aceito os Termos e a Política de Privacidade.</span>
                                            </label>
                                        </div>
                                ...
                                {step === 3 && (
                                    <div className="text-center space-y-6 bg-slate-800 p-8 rounded-2xl border border-slate-700">
                                        ...
                                        <p className="text-slate-400">Conclua o pagamento de R$ 39,90 para liberar a geração automática.</p>
                                        ...
                                        <button onClick={() => setStep(1)} className="text-yellow-500 font-bold text-sm">JÁ PAGOU? CLIQUE AQUI PARA COMEÇAR</button>
                                    </div>
                                )}

                                {step === 1 && (
                                    <div className="space-y-6">
                                        <h2 className="text-2xl font-bold text-center">Sobre seu Livro</h2>
                                        <div className="space-y-4 bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
                                            <input value={bookData.authorName} onChange={e => setBookData({...bookData, authorName: e.target.value})} className="w-full bg-slate-900 border-slate-700 rounded-xl p-3" placeholder="Nome do Autor" />
                                            <textarea value={bookData.topic} onChange={e => setBookData({...bookData, topic: e.target.value})} className="w-full bg-slate-900 border-slate-700 rounded-xl p-3 h-32" placeholder="Tema/Assunto do Livro..." />
                                        </div>
            ...
            <section id="offer" className="py-24 bg-slate-800/30 border-y border-slate-800">
                <div className="max-w-4xl mx-auto px-6">
                    <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-[2.5rem] p-10 md:p-16 border border-slate-700 shadow-2xl text-center">
                        ...
                        <h2 className="text-4xl md:text-5xl font-black mb-6">Gere Seu Futuro Best Seller AGORA</h2>
                        <div className="flex items-center justify-center gap-4 mb-10">
                            <span className="text-7xl font-black">R$ 39,90</span>
                        </div>
                        <ul className="text-left space-y-4 max-w-md mx-auto mb-10">
                            {['1 Livro Completo com IA', 'Diagramação Profissional Inclusa', 'Exportação em Word Editável', 'Pesquisa Avançada de Temas'].map((f, i) => (
            ...
            <footer className="py-12 border-t border-slate-800">
                <div className="max-w-7xl mx-auto px-6 text-center">
                    ...
                    <p className="text-slate-500 text-sm mb-6">© 2026 Todos os direitos reservados.</p>
                    <Disclaimer />
                </div>
            </footer>
