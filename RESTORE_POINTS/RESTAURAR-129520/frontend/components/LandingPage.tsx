...
const LandingPage: React.FC<LandingProps> = ({ onStart, onAdmin, lang, setLang }) => {
    ...
    useEffect(() => {
        const d = localStorage.getItem('activeDiscount');
    ...
    useEffect(() => {
        ...
        if (giftCode) {
            try {
                ...
                // Auto-check validity
                fetch(`http://localhost:3001/api/payment/access?email=${sourceEmail}&_t=${Date.now()}`)
                    .then(res => res.json())
                    .then(data => {
    ...
    const handleSaveLead = async (overrideType?: string) => {
        try {
            const payload = { ...formData, ...bookData, type: overrideType || formData.type };
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const res = await fetch('http://localhost:3001/api/projects/upload-existing', {
                method: 'POST',
                body: fd
            });
            const data = await res.json();
            if (res.ok) {
                setUploadData(prev => ({ ...prev, name: '', email: '', phone: '', file: null })); // Reset
                setIsUploadModalOpen(false);

                // Set main form data to match upload for payment check
                setFormData({
    ...
    useEffect(() => {
        let interval: any;
        // Only poll if we are in paywall step AND NOT redeeming a voucher (voucher redemption is manual click)
        if (step === 3 && !giftSourceEmail) {
            interval = setInterval(async () => {
                try {
                    const res = await fetch(`http://localhost:3001/api/payment/access?email=${formData.email.trim()}&_t=${Date.now()}`);
                    const data = await res.json();
                    // For Voucher, we specifically need CREDITS to be > 0. 
                    // hasAccess might be true if they have an active project, but that doesn't mean they bought the voucher.
                    if (formData.type === 'VOUCHER') {
                        // STRICT CHECK: The latest lead must be APPROVED. 
                        // This ensures we are validating the CURRENT transaction, not just prior credits.
                        if (data.leadStatus === 'APPROVED' && data.credits > 0) {
                    ...
                    // DIAGRAMMING FLOW
                    if (formData.type === 'DIAGRAMMING') {
                        if (data.leadStatus === 'APPROVED') {
                            clearInterval(interval);
                            setStep(5); // Go to Diagramming Process
                        }
                        return;
                    }
                    ...
                    if (data.hasAccess) {
                        ...
                        // RESUME LOGIC: If active project exists, skip credit consumption
                        if (data.hasActiveProject) {
                            console.log("Resuming active project...");
                            onStart(formData, bookData);
                            return;
                        }

                        // Normal flow: Consume and Start
                        const consumeRes = await fetch(`http://localhost:3001/api/payment/use`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
        ...
        return () => clearInterval(interval);
    }, [step, formData.email, formData.type, giftSourceEmail]);
    ...
    return (
        <div className="min-h-screen font-sans bg-slate-900 text-slate-100 selection:bg-yellow-500 selection:text-slate-900 overflow-x-hidden">
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="bg-gradient-to-br from-emerald-900/30 to-emerald-800/20 p-6 rounded-2xl border border-emerald-500/30 space-y-4 hover:border-emerald-400/50 transition-all">
                                                ...
                                                <div className="bg-emerald-950/40 p-5 rounded-xl border border-emerald-500/20 mt-2 space-y-3">
                                                    ...
                                                    <button
                                                        className="w-full px-4 py-4 rounded-xl font-bold text-sm uppercase tracking-wider bg-emerald-600/90 hover:bg-emerald-500 text-white transition-all shadow-lg shadow-emerald-900/20 flex items-center justify-center gap-2 mt-4 hover:scale-[1.02]"
                                                        onClick={async () => {
                                            ...
                                            <button
                                                onClick={async () => {
                                                    // Same logic as old "nextStep" but merged
                                                ...
                                                disabled={!formData.name || !formData.email || !formData.phone || !bookData.authorName || !bookData.topic}
                                    ...
                                    <div className="bg-slate-800 p-8 rounded-2xl border border-slate-700 shadow-2xl relative overflow-hidden animate-fade-in">
                                        ...
                                        {giftSourceEmail ? (
                                                ...
                                                <div className="mt-8">
                                                    {voucherCredits > 0 ? (
                                                        ...
                                                        <div className="bg-red-500/20 text-red-400 p-4 rounded-xl border border-red-500/30">
                                                            Poxa! O crédito deste link já foi utilizado ou expirou.
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <>
                                                ...
                                                {(() => {
                                                    const price = activeDiscount > 0 ? (24.99 * (1 - activeDiscount / 100)) : 24.99;
                                                    const finalPriceStr = price.toFixed(2).replace('.', ',');
                                                    return (
                                                        <button
                                                            onClick={() => window.open(activeDiscount > 0 ? 'https://pay.kiwify.com.br/SpCDp2q?discount=' + activeDiscount : 'https://pay.kiwify.com.br/SpCDp2q', '_blank')}
                                                        ...
                                                        >
                                                            {activeDiscount > 0 && <span className="block text-xs opacity-80 line-through">De R$ 24,99 por</span>}
                                                            PAGAR R$ {finalPriceStr} E LIBERAR
                                                        </button>
                                                    );
                                                })()}
                                    ...
                                    <div className="bg-slate-800 p-8 rounded-2xl border border-slate-700 shadow-2xl animate-fade-in w-full max-w-2xl">
                                        ...
                                        {diagramStep === diagramMessages.length - 1 && (
                                            <div className="mt-10 text-center animate-fade-in-up">
                                                <button
                                                    onClick={() => window.open(`http://localhost:3001/api/admin/books/${formData.email}`, '_blank')}
                    ...
                    <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                        ...
                        <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-3xl p-8 md:p-12 text-center shadow-2xl shadow-yellow-500/20 text-slate-900 transform hover:scale-[1.02] transition-transform relative overflow-hidden">
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="text-6xl font-black mb-8 tracking-tighter">R$ 24,99</div>
                                <button
                                    onClick={startWizard}
                        ...
                        <div className="bg-slate-800 border-2 border-slate-700 rounded-3xl p-8 md:p-12 text-center shadow-2xl relative overflow-hidden group hover:border-green-500/50 transition-colors">
                            ...
                            <div className="text-6xl font-black text-white mb-8 tracking-tighter">R$ 24,99</div>
            ...
            <section className="py-20 border-t border-slate-800 bg-slate-900/50">
                <div className="max-w-6xl mx-auto px-6">
                    <div className="text-center mb-16">
                        <span className="text-yellow-400 font-bold tracking-widest uppercase text-sm">Serviços Premium</span>
                        <h2 className="text-3xl md:text-5xl font-bold mt-2 mb-4">Acelere sua Carreira de Autor</h2>
                        <p className="text-xl text-slate-400 max-w-3xl mx-auto">Serviços profissionais para quem não brinca em serviço.</p>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        {products.english_book && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-indigo-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-indigo-400">Tradução para Inglês 🇺🇸</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Venda no mercado global (Amazon.com).</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 24,99</span>
                                    <a href={products.english_book} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-indigo-600 text-white rounded-lg font-bold text-sm transition-colors">Comprar</a>
                                </div>
                            </div>
                        )}
                        {products.spanish_book && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-indigo-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-indigo-400">Tradução para Espanhol 🇪🇸</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Conquiste leitores na América Latina e Espanha.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 24,99</span>
                                    <a href={products.spanish_book} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-indigo-600 text-white rounded-lg font-bold text-sm transition-colors">Comprar</a>
                                </div>
                            </div>
                        )}
                        ...
                        {products.cover_printed && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-yellow-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-yellow-400">Capa Livro Impresso</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Capa, contracapa + orelhas (7cm). Design Pro.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 250,00</span>
                                    <a href={products.cover_printed} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-yellow-600 text-white rounded-lg font-bold text-sm transition-colors">Comprar</a>
                                </div>
                            </div>
                        )}
                        {products.cover_ebook && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-yellow-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-yellow-400">Capa Ebook (Kindle)</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Design otimizado para lojas digitais.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 99,90</span>
                                    <a href={products.cover_ebook} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-yellow-600 text-white rounded-lg font-bold text-sm transition-colors">Comprar</a>
                                </div>
                            </div>
                        )}
                        ...
                        {products.pub_amazon_printed && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-green-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-green-400">Publicação Amazon (Impresso)</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Colocamos seu livro físico à venda na maior loja do mundo.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 49,90</span>
                                    <a href={products.pub_amazon_printed} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-green-600 text-white rounded-lg font-bold text-sm transition-colors">Contratar</a>
                                </div>
                            </div>
                        )}
                        {products.pub_amazon_digital && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-green-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-green-400">Publicação Amazon (Ebook)</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Publique no Kindle Direct Publishing (KDP).</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 39,90</span>
                                    <a href={products.pub_amazon_digital} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-green-600 text-white rounded-lg font-bold text-sm transition-colors">Contratar</a>
                                </div>
                            </div>
                        )}
                        {products.pub_uiclap && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-green-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-green-400">Publicação UICLAP</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Venda impresso no Brasil sem estoque.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 49,90</span>
                                    <a href={products.pub_uiclap} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-green-600 text-white rounded-lg font-bold text-sm transition-colors">Contratar</a>
                                </div>
                            </div>
                        )}
                        ...
                        {products.catalog_card && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-blue-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-blue-400">Ficha Catalográfica</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Documento obrigatório para bibliotecas e livrarias.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 59,90</span>
                                    <a href={products.catalog_card} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-blue-600 text-white rounded-lg font-bold text-sm transition-colors">Gerar</a>
                                </div>
                            </div>
                        )}
                        {products.isbn_printed && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-blue-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-blue-400">Registro ISBN (Impresso)</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">O "CPF" do seu livro físico. Oficial Câmara Brasileira.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 49,90</span>
                                    <a href={products.isbn_printed} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-blue-600 text-white rounded-lg font-bold text-sm transition-colors">Registrar</a>
                                </div>
                            </div>
                        )}
                        ...
                        {products.sales_page && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-pink-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-pink-400">Página de Vendas Pro</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Landing Page de alta conversão para seu lançamento.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 349,90</span>
                                    <a href={products.sales_page} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-pink-600 text-white rounded-lg font-bold text-sm transition-colors">Ver Mais</a>
                                </div>
                            </div>
                        )}
                        {products.hosting && (
                            <div className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-pink-500/50 transition group">
                                <h3 className="font-bold text-lg mb-2 text-white group-hover:text-pink-400">Hospedagem Anual</h3>
                                <p className="text-sm text-slate-400 mb-4 h-10">Servidor rápido e seguro para seus projetos.</p>
                                <div className="flex justify-between items-center mt-auto">
                                    <span className="text-xl font-bold text-white">R$ 499,90</span>
                                    <a href={products.hosting} target="_blank" className="px-4 py-2 bg-slate-700 hover:bg-pink-600 text-white rounded-lg font-bold text-sm transition-colors">Assinar</a>
                                </div>
                            </div>
                        )}
                        ...
                        {products.complete_package && (
                            <div className="col-span-1 md:col-span-2 lg:col-span-3 bg-gradient-to-r from-yellow-600 to-yellow-500 text-slate-900 p-8 rounded-2xl relative overflow-hidden shadow-2xl transform hover:scale-[1.01] transition-all">
                                <div className="absolute top-0 right-0 bg-slate-900 text-yellow-400 text-xs font-bold px-4 py-2 rounded-bl-xl uppercase tracking-widest">Melhor Valor</div>
                                <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                                    <div className="text-left">
                                        <h3 className="text-3xl font-black mb-2">PACOTE COMPLETO EDITORA</h3>
                                        <p className="font-medium text-slate-800 mb-4 max-w-xl">Tenha tudo o que precisa: Capa Profissional + Publicação Amazon/UICLAP + Ficha Catalográfica + ISBN e Suporte Premium.</p>
                                        <div className="flex gap-2">
                                    ...
                                    <div className="text-center md:text-right min-w-[200px]">
                                        <div className="text-sm opacity-80 line-through font-bold">de R$ 899,90</div>
