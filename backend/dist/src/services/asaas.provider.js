...
if (!process.env.ASAAS_API_KEY)
    console.warn("WARN: Using Hardcoded Sandbox Key. Configure ASAAS_API_KEY in env for custom accounts.");
const getApi = () => {
...
exports.AsaasProvider = {
    ...
    getCustomerByEmail(email) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { data } = yield getApi().get(`/customers?email=${email}`);
                if (data.data && data.data.length > 0)
                    return data.data[0];
                return null;
            }
    ...
    getPayments(params) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const { data } = yield getApi().get(`/payments?${qs}`);
                return data.data || [];
            }
            catch (error) {
