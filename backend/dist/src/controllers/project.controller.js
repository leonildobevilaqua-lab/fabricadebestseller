...
const create = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
            ...
            if (existing && existing.metadata.status !== 'COMPLETED' && existing.metadata.status !== 'FAILED') {
                ...
                if (existing.metadata.topic === 'Livro Pré-Escrito' && topic !== 'Livro Pré-Escrito') {
                    ...
                    if (existing.metadata.status === 'IDLE') {
                        yield QueueService.updateMetadata(existing.id, { authorName, topic, language, contentStyle, writingTone });
        ...
        // ---------------------------
        const project = yield QueueService.createProject({ authorName, topic, language, contact, contentStyle, writingTone });
        ...
        if (!isResuming && safeEmail) {
            try {
                const userOrders = (yield (0, db_service_1.getVal)(`/users/${safeEmail}/orders`)) || [];
                // Find oldest AVAILABLE credit
                // We look for 'CREDIT_AVAILABLE' which was created by Payment Controller
                let orderIndex = userOrders.findIndex((o) => o.status === 'CREDIT_AVAILABLE');
                ...
                if (orderIndex !== -1) {
                    ...
                    const updatedOrder = Object.assign(Object.assign({}, userOrders[orderIndex]), { status: 'PROCESSING', title: topic || "Livro em Produção...", projectId: project.id // LINK IT!
                     });
                    yield (0, db_service_1.setVal)(`/users/${safeEmail}/orders[${orderIndex}]`, updatedOrder);
                }
            }
            catch (e) {
...
const startResearch = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const pStatus = project.metadata.status;
    ...
    const userEmail = ((_a = project.metadata.contact) === null || _a === void 0 ? void 0 : _a.email) || bodyEmail;
    if (userEmail) {
        ...
        // LOCALHOST BYPASS
        const isLocal = ((_b = req.headers.host) === null || _b === void 0 ? void 0 : _b.includes('localhost')) || ((_c = req.headers.host) === null || _c === void 0 ? void 0 : _c.includes('127.0.0.1'));
        if (isLocal) {
            hasAccess = true;
        ...
        if (!hasAccess) {
            ...
            const ledgerCredits = Number((yield (0, db_service_1.getVal)(`/credits/${safeEmail}`)) || 0);
            if (ledgerCredits > 0) {
                hasAccess = true;
            ...
            if (!hasAccess) {
                ...
                for (const l of leads) {
                    if (((_d = l.email) === null || _d === void 0 ? void 0 : _d.toLowerCase().trim()) === cleanUserEmail) {
            ...
            if (!hasAccess) {
                try {
                    ...
                    let customer = yield asaas_provider_1.AsaasProvider.getCustomerByEmail(originalEmail);
                    if (!customer && originalEmail !== cleanEmail) {
                        console.log(`[startResearch] Customer not found with ${originalEmail}, trying ${cleanEmail}...`);
                        customer = yield asaas_provider_1.AsaasProvider.getCustomerByEmail(cleanEmail);
                    }
                    if (customer) {
                        const payments = yield asaas_provider_1.AsaasProvider.getPayments({ customer: customer.id, limit: 50 });
    ...
    try {
        const userEmail = (_e = project.metadata.contact) === null || _e === void 0 ? void 0 : _e.email;
        if (userEmail) {
            const rawLeads = (yield (0, db_service_1.getVal)('/leads')) || [];
            const leads = Array.isArray(rawLeads) ? rawLeads : Object.values(rawLeads);
            for (let i = leads.length - 1; i >= 0; i--) {
                if (((_f = leads[i].email) === null || _f === void 0 ? void 0 : _f.toLowerCase().trim()) === userEmail.toLowerCase().trim()) {
    ...
    (() => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            let googleResearch = "";
            try {
                googleResearch = yield AIService.researchGoogle(topic, ytResearch, targetLang);
            }
            catch (googleError) {
            ...
            let compResearch = "";
            try {
                compResearch = yield AIService.analyzeCompetitors(topic, ytResearch + "\n" + googleResearch, targetLang);
            }
            catch (compError) {
...
const selectTitle = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const structure = yield AIService.generateStructure(title, subtitle, project.researchContext, lang, project.metadata.contentStyle);
...
const generateBookContent = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (!hasIntro) {
            ...
            let introContent = "";
            try {
                introContent = yield AIService.writeIntroduction(project.metadata, project.structure, project.researchContext, targetLang);
            }
            catch (e) {
